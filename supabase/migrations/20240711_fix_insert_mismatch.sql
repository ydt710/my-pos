-- Corrects the INSERT statement in the pay_order function.
...
-- Re-create the function with the definitive corrected logic.
...
DECLARE
    ...
    v_cash_kept numeric;
BEGIN
    -- Step 1: Validate stock for each item (unchanged).
    ...
    -- Step 2: Insert the order (unchanged).
    ...
    -- Step 3: Insert order items and decrease stock (unchanged).
    FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
        v_product_id := (v_item->>'product_id')::uuid;
        v_quantity := (v_item->>'quantity')::int;
    ...
    -- Step 4: Calculate the final change in the user's balance.
    v_balance_change := p_payment_amount - (p_order_total - p_credit_used);
    
    -- Step 5: Calculate cash kept for stats and adjust balance change if giving change.
    v_cash_kept := p_payment_amount;
    IF p_extra_cash_option = 'change' THEN
        DECLARE
            v_change_given numeric := GREATEST(0, p_payment_amount + p_credit_used - p_order_total);
        BEGIN
            v_cash_kept := p_payment_amount - v_change_given;
            v_balance_change := v_balance_change - v_change_given;
        END;
    END IF;

    -- Step 6: Log a single, consolidated transaction for this order.
    INSERT INTO transactions (user_id, order_id, amount, method, type, note, order_cost, payment_amount, credit_used)
    VALUES (p_user_id, v_order_id, v_balance_change, p_method, 'order', 'Order ' || v_order_id, p_order_total, v_cash_kept, p_credit_used);
    
    -- Step 7: Update user balance in the profiles table.
