-- =============================================
-- EXPORT SCRIPT 07a: STORAGE BUCKETS
-- =============================================
-- Run this script after 06e_functions_utils.sql.
-- This script creates storage buckets and basic policies.
-- =============================================

-- Storage Bucket Creation (idempotent due to ON CONFLICT)
...
CREATE POLICY "Allow public read access to product images" ON storage.objects FOR SELECT TO anon, authenticated USING (bucket_id = 'products');

DROP POLICY IF EXISTS "Allow admin to manage product images" ON storage.objects;
CREATE POLICY "Allow admin to manage product images" ON storage.objects FOR ALL USING (bucket_id = 'products' AND public.is_admin());

DROP POLICY IF EXISTS "Users can upload their own signatures" ON storage.objects;
CREATE POLICY "Users can upload their own signatures" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'signatures' AND auth.uid()::text = (storage.foldername(name))[1]);

DROP POLICY IF EXISTS "Users can view their own signatures" ON storage.objects;
CREATE POLICY "Users can view their own signatures" ON storage.objects FOR SELECT TO authenticated USING (bucket_id = 'signatures' AND auth.uid()::text = (storage.foldername(name))[1]);

DROP POLICY IF EXISTS "Admins can view all signatures" ON storage.objects;
CREATE POLICY "Admins can view all signatures" ON storage.objects FOR SELECT TO authenticated USING (bucket_id = 'signatures' AND public.is_admin());

DROP POLICY IF EXISTS "Users can upload their own ID images" ON storage.objects;
CREATE POLICY "Users can upload their own ID images" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'id_images' AND auth.uid()::text = (storage.foldername(name))[1]);

DROP POLICY IF EXISTS "Users can view their own ID images" ON storage.objects;
CREATE POLICY "Users can view their own ID images" ON storage.objects FOR SELECT TO authenticated USING (bucket_id = 'id_images' AND auth.uid()::text = (storage.foldername(name))[1]);

DROP POLICY IF EXISTS "Admins can view all ID images" ON storage.objects;
CREATE POLICY "Admins can view all ID images" ON storage.objects FOR SELECT TO authenticated USING (bucket_id = 'id_images' AND public.is_admin());
