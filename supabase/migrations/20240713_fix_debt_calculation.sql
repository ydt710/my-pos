-- Corrects the pay_order function to properly handle debt creation.
...
-- Re-create the function with corrected balance logic.
...
BEGIN
    ...
    -- Step 3: Insert the order with the new order number.
    ...
    -- Step 6: Calculate cash kept for stats and determine change.
    v_cash_kept := p_payment_amount;
    v_change_given := GREATEST(0, v_balance_change);
    
    IF p_extra_cash_option = 'change' AND v_change_given > 0 THEN
      -- If giving change, cash kept is payment minus change.
      v_cash_kept := p_payment_amount - v_change_given;
      -- Balance change for the user becomes 0 because they got the excess back.
      v_balance_change := 0;
    END IF;
    -- If the option is 'credit', v_balance_change remains positive and is added to their balance.

    -- Step 7: Log a single, consolidated transaction for this order.
    ...
    -- Step 8: Update user balance in the profiles table.
