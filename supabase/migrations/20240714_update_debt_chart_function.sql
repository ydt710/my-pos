-- Updates the debt chart function to use the new debt_paid_amount column for accuracy.
...
BEGIN
    ...
    transactions_data AS (
        SELECT
            ...
            -- Debt created is the sum of all negative transaction amounts.
            SUM(CASE WHEN amount < 0 THEN ABS(amount) ELSE 0 END) AS total_debt_created,
            -- Debt paid is the sum of explicit debt payments (from orders) and admin payments.
            SUM(
